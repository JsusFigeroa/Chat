...
pub(super) fn generate_not_identified_msg() -> Vec<u8> {
    let msg = TypeSendMessages::Response {
        operation: Operations::Invalid,
        result: Resultado::NotIdentified,
        extra: None,
    };
    let mut msg_to_bytes =
        serde_json::to_vec(&msg).expect("No fue posible generar el mensaje de identificación");
    msg_to_bytes.push(b'\n');
    msg_to_bytes
}
...
pub(super) fn generate_text_user_not_exist(username: &str) -> Vec<u8> {
    let message = TypeSendMessages::Response {
...
pub(super) fn generate_disconected_msg(username: &str) -> Vec<u8> {
    let msg = TypeSendMessages::Disconnected {
...
pub(super) fn generate_public_text_from(username: &str, text: &str) -> Vec<u8> {
    let msg = TypeSendMessages::PublicTextFrom {
        username: username.to_string(),
        text: text.to_string(),
    };
    let mut msg_to_bytes = serde_json::to_vec(&msg).unwrap();
...
pub(super) fn generate_new_status_msg(username: &str, status: State) -> Vec<u8> {
    let message = TypeSendMessages::NewStatus {
        username: username.to_string(),
        status,
    };
    let mut msg =
        serde_json::to_vec(&message).expect("No se pudo generar el mensaje de nuevo status");
    msg.push(b'\n');
    msg
}
...
pub(super) fn generate_text_from_msg(username: &str, text: &str) -> Vec<u8> {
    let message = TypeSendMessages::TextFrom {
        username: username.to_string(),
        text: text.to_string(),
    };
...
pub(super) fn generate_new_user_msg(username: &str) -> Vec<u8> {
    let message = TypeSendMessages::NewUser {
        username: username.to_string(),
    };
...
pub(crate) fn generate_invitation_msg(username: &str, roomname: &str) -> Vec<u8> {
    let message = TypeSendMessages::Invitation {
        username: username.to_string(),
...
pub(crate) fn generate_new_room_user_msg(roomname: &str, new_user: &str) -> Vec<u8> {
    let message = TypeSendMessages::JoinedRoom {
        roomname: roomname.to_string(),
...
pub(crate) fn generate_room_users_msg(
    roomname: &str,
    users_map: HashMap<String, State>,
) -> Vec<u8> {
    let message = TypeSendMessages::RoomUserList {
        roomname: roomname.to_string(),
        users: users_map,
    };
    let mut msg = serde_json::to_vec(&message).unwrap();
...
pub(crate) fn generate_user_leaved_room(username: &str, roomname: &str) -> Vec<u8> {
    let message = TypeSendMessages::LeftRoom {
        roomname: roomname.to_string(),
        username: username.to_string(),
    };
    let mut msg = serde_json::to_vec(&message).unwrap();
...
pub(crate) fn leave_room_not_such_room(roomname: &str) -> Vec<u8> {
    let message = TypeSendMessages::Response {
        operation: Operations::LeaveRoom,
        result: Resultado::NoSuchRoom,
        extra: Some(roomname.to_string()),
    };
    let mut msg = serde_json::to_vec(&message).unwrap();
...
pub(crate) fn room_users_not_joined(roomname: &str) -> Vec<u8> {
    let message = TypeSendMessages::Response {
        operation: Operations::RoomUsers,
        result: Resultado::NotJoined,
        extra: Some(roomname.to_string()),
    };
    let mut msg = serde_json::to_vec(&message).unwrap();
...
pub(crate) fn generate_room_text_from(username: &str, roomname: &str, text: &str) -> Vec<u8> {
    let message = TypeSendMessages::RoomTextFrom {
        roomname: roomname.to_string(),
        username: username.to_string(),
        text: text.to_string(),
    };
